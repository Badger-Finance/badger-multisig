...
def main(simulation="false"):
    ...
    deployers = [wallet for wallet in r.badger_wallets.keys() if "deployer" in wallet]
    ...
    if simulation == "true":
        ...
        registry_acl.add(
            DUMMY_VAULT,
            "v1.5",
            "name=BTC-CVX,protocol=Badger,behavior=DCA",
            {"from": deployer},
        )
        assert registry.getVaults("v1.5", registry_acl) == [
            [DUMMY_VAULT, "v1.5", "1", "name=BTC-CVX,protocol=Badger,behavior=DCA"]
        ]

        # Test promote
        registry_acl.promote(
            DUMMY_VAULT,
            "v1.5",
            "name=BTC-CVX,protocol=Badger,behavior=DCA",
            3,
        ...
        assert [
            ...
            "1",
            "name=BTC-CVX,protocol=Badger,behavior=DCA",
        ] not in registry.getFilteredProductionVaults("v1.5", 3)
        assert [
            DUMMY_VAULT,
            "v1.5",
            "1",
            "name=BTC-CVX,protocol=Badger,behavior=DCA",
        ] in registry.getFilteredProductionVaults("v1.5", 1)
    else:
