...
def test_deposit_and_stake_single_asset(safe, balancer, wbtc, bpt, staked_bpt):
    ...
    balancer.deposit_and_stake_single_asset(wbtc, amount_wbtc, bpt)
    ...
    with pytest.raises(NotImplementedError):
        with brownie.reverts():
            balancer.deposit_and_stake_single_asset(
                dai, dai.balanceOf(safe) / 4, threepool_bpt
            )
...
def test_deposit_and_stake_weighted(
    safe, balancer, dai, weighted_bpt, weighted_staked_bpt
):
    bal_before_staked_bpt = weighted_staked_bpt.balanceOf(safe)
    bal_before_dai = dai.balanceOf(safe)

    balancer.deposit_and_stake_single_asset(dai, 1000e18, weighted_bpt)
...
def test_deposit_and_stake_weighted_eth(
    safe, balancer, dai, weth, weighted_bpt, weighted_staked_bpt
):
    bal_before_staked_bpt = weighted_staked_bpt.balanceOf(safe)
    ...
    with brownie.reverts():
        balancer.deposit_and_stake(
            [dai, weth], [int(1e18), 100], pool=weighted_bpt, is_eth=True
        )
