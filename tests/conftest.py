...
@pytest.fixture
def bpt(dev):
    return dev.contract(registry_addr.eth.balancer['50-BTC-50-WETH'])


@pytest.fixture
def staked_bpt(dev):
    return dev.contract(registry_addr.eth.balancer['50-BTC-50-WETH-G'])


@pytest.fixture
def wbtc(dev):
    Contract.from_explorer(registry_addr.eth.treasury_tokens.WBTC)
    wbtc = MintableForkToken(
        registry_addr.eth.treasury_tokens.WBTC, owner=dev.account
    )
    wbtc._mint_for_testing(dev, 1000 * 10**wbtc.decimals())
    return Contract(
        registry_addr.eth.treasury_tokens.WBTC, owner=dev.account
    )
...
@pytest.fixture
def weth(dev):
    Contract.from_explorer(registry_addr.eth.treasury_tokens.WETH)
    weth = MintableForkToken(
        registry_addr.eth.treasury_tokens.WETH, owner=dev.account
    )
    weth._mint_for_testing(dev, 1000 * 10**weth.decimals())
    return Contract(
        registry_addr.eth.treasury_tokens.WETH, owner=dev.account
    )
